...
export default function RsvpTab({
  ...
  rsvps: { eventMemberId: string; attendanceStatus: 'ATTENDING' | 'DECLINED' | 'MAYBE'; notes: string | null; adultCount: number; childCount: number }[]
}) {
          ...
          return (
            <div key={member.id} className="py-4 flex items-start gap-3 first:pt-0">
              <div className={cn(
                ...
                status === 'ATTENDING' ? 'bg-emerald-50' : status === 'MAYBE' ? 'bg-amber-50' : status === 'DECLINED' ? 'bg-rose-50' : 'bg-surface-muted',
              )}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-muted mt-0.5">
                  {status ? t(`rsvpStatus.${status}`) : t('rsvpStatus.NO_RESPONSE')}
                </p>
