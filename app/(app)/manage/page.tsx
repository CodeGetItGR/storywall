...
import { Users, Ticket, LayoutDashboard, CalendarPlus, Settings, Loader2 } from 'lucide-react'
import { useTranslations } from 'next-intl'
import { cn } from '@/lib/utils'
import { useActiveEvent, useIsHost, useEventContextLoading } from '@/providers/EventProvider'
...
export default function ManagePage() {
  ...
  useEffect(() => {
    ...
    if (!eventId) { router.replace('/welcome'); return }
  ...
  const daysToGo = Math.max(0, Math.ceil(
    (new Date(activeEvent.schedule.startAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24),
  ))
    ...
    <div className="max-w-3xl mx-auto pb-24 lg:pb-8">
      ...
      <div className="px-4 pt-5 pb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <span className="px-3 py-1 rounded-full bg-primary-light text-primary-dark text-xs font-bold whitespace-nowrap">{t('hostView')}</span>
        </div>
      </div>
