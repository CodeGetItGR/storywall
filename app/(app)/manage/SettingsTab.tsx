...
const inputClass = 'bg-surface-muted rounded-xl px-4 py-3 text-sm text-ink placeholder:text-ink-faint outline-none focus:ring-2 focus:ring-primary/30 transition'
...
export default function SettingsTab({
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setSaved(false)
    uploadMedia.mutate(
      { eventId: event.id, file, mediaType: 'IMAGE' },
  ...
  return (
    <div className="px-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {updateEvent.isError && !fieldErrors && (
          <p className="text-xs text-rose-500">{getErrorMessage(updateEvent.error)}</p>
        )}
