...
export default function OverviewTab({
  t, memberCount, daysToGo, invitationCount, rsvpBreakdown, onSeeAllRsvp, onSeeAllInvitations,
}: {
  ...
  return (
    <div className="px-4 flex flex-col gap-6">
      <div className="grid grid-cols-3 gap-4">
        <Stat
          label={t('stats.totalGuests.label')} value={`${memberCount}`}
          sub={t('stats.totalGuests.sub')}
          color="bg-emerald-50 text-emerald-600"
          Icon={Users}
        />
        <Stat
          label={t('stats.daysToGo.label')} value={`${daysToGo}`}
          sub={t('stats.daysToGo.sub')}
          color="bg-rose-50 text-rose-500"
          Icon={Clock}
        />
        <Stat
          label={t('stats.invitations.label')} value={`${invitationCount}`}
      ...
      <Section
        ...
        action={<button onClick={onSeeAllRsvp} className="text-xs text-primary font-semibold hover:underline">{t('seeAll')}</button>}
      >
      ...
      <Section
        ...
        action={<button onClick={onSeeAllInvitations} className="text-xs text-primary font-semibold hover:underline">{t('seeAll')}</button>}
      >
