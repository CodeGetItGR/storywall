...
import {
  ...
  useUpdateEventInvitation,
  useDeleteEventInvitation,
...
export default function InvitationsTab({
  ...
  return (
    <div className="px-4 flex flex-col">
      ...
      {showCreate && (
        <CreateInvitationForm t={t} eventId={eventId} onDone={() => setShowCreate(false)} />
      )}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 py-4 border-y border-border mb-1">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={onDone} aria-label={t('invitations.create.cancel')} className="text-ink-muted hover:text-ink transition-colors">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {createInvitation.isError && !fieldErrors && (
        <p className="text-xs text-rose-500">{getErrorMessage(createInvitation.error)}</p>
      )}
    ...
    <div className="py-4 flex flex-col gap-2 first:pt-0">
        ...
        <div className="min-w-0">
          ...
          {(guestName || invitation.email) && (
            <p className="text-xs text-ink-muted mt-0.5 truncate">
              {[guestName, invitation.email].filter(Boolean).join(' · ')}
            </p>
          )}
        </div>
      </div>
      ...
      {isEditing ? (
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => { setIsEditing(false); setMaxGuests(invitation.maxGuests) }}
