...
export default function SeatingPage() {
  ...
  const allGuests = seatingTables.flatMap(t =>
    t.guests.map(g => ({ ...g, tableName: t.name, tableId: t.id }))
  )

  const searchResults = search.trim()
    ? allGuests.filter(g => g.name.toLowerCase().includes(search.toLowerCase()))
    : []
        ...
        <div className="mb-5">
          {searchResults.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {searchResults.map(g => (
                <div key={g.id} className="flex items-center justify-between bg-primary-light rounded-xl px-4 py-3 border border-primary/20">
                  <div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div
                  className={cn('w-5 h-5 flex items-center justify-center text-ink-faint transition-transform', isExpanded && 'rotate-180')}
                  aria-hidden="true"
                >
