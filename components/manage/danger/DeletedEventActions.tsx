...
export function DeletedEventActions() {
    ...
    return (
        <section>
            <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-ink-muted">{t('settings.deleted.actionsTitle')}</h2>
            <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-background">
                {items.map(({ key, href, icon: Icon, label }) => (
                    <Link
                        key={key}
                        href={href}
                        className="group flex min-h-16 items-center gap-3 px-4 py-3 transition-colors hover:bg-surface-muted"
                    >
