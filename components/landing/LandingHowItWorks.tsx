...
export function LandingHowItWorks() {
    ...
    return (
        <section aria-labelledby="how-it-works-title" className="bg-white px-5 py-[34px] pb-[38px] text-[#151313] min-[761px]:px-[clamp(28px,5.5vw,96px)] min-[761px]:py-[clamp(30px,3.2vw,48px)]" id="howItWorks">
                ...
                {steps.map((step, index) => (
                    ...
                    >
                        <div className="mb-2.5 inline-flex items-center gap-2 min-[761px]:gap-[9px]">
                            <span className="bg-[linear-gradient(90deg,#f2c764_0%,#f09a62_55%,#df7794_100%)] bg-clip-text text-[12px] font-black tracking-[0.08em] text-transparent">0{index + 1}</span>
                            <svg aria-hidden="true" className="size-5 fill-none stroke-[rgba(220,121,134,0.72)] stroke-[1.35] min-[761px]:size-[21px]" viewBox="0 0 24 24">
                                {STEP_ICONS[index]}
                            </svg>
                        </div>
                        <h3 className="m-0 text-[31px] leading-[0.98] font-normal tracking-[-0.03em] [font-family:var(--editorial)] min-[761px]:text-[clamp(27px,2vw,34px)]">{step.title}</h3>
                        <p className="mt-2.5 max-w-full text-[17px] leading-[1.5] min-[761px]:mt-[10px] min-[761px]:max-w-[330px] min-[761px]:text-[15px] min-[761px]:leading-[1.45]">{step.description}</p>
                    </article>
                ))}
            </div>
        </section>
