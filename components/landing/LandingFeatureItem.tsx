    ...
    return (
        ...
        >
            ...
            >
                <span
                    aria-hidden="true"
                    className="block size-full [transform:translateZ(0)] bg-[linear-gradient(135deg,#f89ab0_0%,#f6a18f_34%,#f4b37b_68%,#f3cf8a_100%)] [mask-image:var(--sw-icon-mask)] [mask-size:contain] [mask-position:center] [mask-repeat:no-repeat] [filter:drop-shadow(0_4px_8px_rgba(230,126,105,0.14))] [-webkit-mask-image:var(--sw-icon-mask)] [-webkit-mask-position:center] [-webkit-mask-repeat:no-repeat] [-webkit-mask-size:contain] [transition:transform_0.32s_cubic-bezier(0.2,0.75,0.2,1),filter_0.32s_ease] group-hover:[transform:translateY(-2px)_scale(1.025)] group-hover:[filter:drop-shadow(0_7px_11px_rgba(230,126,105,0.2))] motion-reduce:transition-none"
                    style={{ '--sw-icon-mask': `url('${iconPath}')` } as React.CSSProperties}
                />
            </div>
            <div className="mx-auto max-w-32 [font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',sans-serif] text-[12px] leading-[1.25] font-light tracking-normal text-white/86 min-[761px]:max-w-[165px] min-[761px]:text-center min-[761px]:text-[clamp(13px,0.9vw,15px)]">
                {label}
            </div>
        </div>
    );
