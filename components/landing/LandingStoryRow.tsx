...
export function LandingStoryRow({ copy, index, media }: LandingStoryRowProps) {
    ...
    return (
        <article
            className={`story-row group grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-3.5 border-b border-[var(--line)] py-[26px] pb-[34px] opacity-100 outline-none [grid-template-areas:'photo_photo'_'copy_arr'] focus-visible:ring-2 focus-visible:ring-[var(--coral)] focus-visible:ring-inset min-[761px]:items-center min-[761px]:gap-x-[clamp(20px,2.5vw,44px)] min-[761px]:gap-y-0 min-[761px]:border-b-0 min-[761px]:py-[clamp(42px,5vw,78px)] min-[761px]:opacity-[0.36] min-[761px]:transition-opacity min-[761px]:duration-300 min-[761px]:[grid-template-areas:unset] min-[761px]:data-[active=true]:opacity-100 ${
                isOdd
        ...
        >
            <div
                className={`mobile-photo-wrap relative mb-4 block h-[60vw] max-h-[410px] min-h-[230px] w-[calc(100vw-40px)] max-w-[calc(100vw-40px)] overflow-hidden bg-[#111] opacity-0 transition-[clip-path,transform,opacity] duration-[1050ms] ease-[cubic-bezier(0.18,0.8,0.2,1)] will-change-transform [grid-area:photo] min-[761px]:m-0 min-[761px]:h-[62.4vh] min-[761px]:max-h-none min-[761px]:min-h-0 min-[761px]:w-full min-[761px]:max-w-none min-[761px]:opacity-100 min-[761px]:![clip-path:none] min-[761px]:!translate-x-0 min-[761px]:[grid-row:1] ${
                    isOdd
                        ? "ml-[-20px] mr-auto max-[760px]:rounded-br-[38px] max-[760px]:[clip-path:inset(0_38%_0_0)] max-[760px]:-translate-x-[34vw] max-[760px]:group-data-[visible=true]:[clip-path:inset(0_0_0_0)] max-[760px]:group-data-[visible=true]:translate-x-0 max-[760px]:group-data-[visible=true]:opacity-100 min-[761px]:rounded-br-[42px] min-[761px]:[grid-column:1]"
                        : "ml-auto mr-[-20px] max-[760px]:rounded-bl-[38px] max-[760px]:[clip-path:inset(0_0_0_38%)] max-[760px]:translate-x-[34vw] max-[760px]:group-data-[visible=true]:[clip-path:inset(0_0_0_0)] max-[760px]:group-data-[visible=true]:translate-x-0 max-[760px]:group-data-[visible=true]:opacity-100 min-[761px]:rounded-bl-[42px] min-[761px]:[grid-column:4]"
                }`}
            >
                <LandingStoryGallery
                    alts={copy.imageAlts}
            ...
            <div
                className={`[grid-area:copy] max-[760px]:translate-y-[18px] max-[760px]:opacity-0 max-[760px]:transition-[opacity,transform] max-[760px]:duration-[700ms] max-[760px]:ease-[cubic-bezier(0.2,0.75,0.2,1)] max-[760px]:group-data-[visible=true]:translate-y-0 max-[760px]:group-data-[visible=true]:opacity-100 max-[760px]:group-data-[visible=true]:delay-[120ms] min-[761px]:[grid-row:1] ${isOdd ? 'min-[761px]:[grid-column:3]' : 'min-[761px]:[grid-column:2]'}`}
            >
                <LandingStoryCopy paragraphSegments={copy.paragraphSegments} tag={copy.tag} titleSegments={copy.titleSegments} wide={wide} />
            </div>
            <div
                className={`arr mt-8 self-start justify-self-end text-[26px] leading-none text-[var(--ink)] transition-[opacity,transform] duration-[350ms] ease-in-out [grid-area:arr] group-data-[active=true]:-translate-y-1 group-data-[active=true]:translate-x-1 max-[760px]:translate-y-[18px] max-[760px]:opacity-0 max-[760px]:duration-[700ms] max-[760px]:ease-[cubic-bezier(0.2,0.75,0.2,1)] max-[760px]:group-data-[visible=true]:translate-y-0 max-[760px]:group-data-[visible=true]:opacity-100 max-[760px]:group-data-[visible=true]:delay-[180ms] min-[761px]:mt-0 min-[761px]:self-start min-[761px]:pt-2 min-[761px]:text-[30px] min-[761px]:[grid-row:1] ${
                    isOdd ? 'min-[761px]:[grid-column:4]' : 'min-[761px]:[grid-column:3]'
                }`}
            >
                ↗
            </div>
