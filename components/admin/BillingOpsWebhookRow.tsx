...
export function BillingOpsWebhookRow({ webhook }: { webhook: UnprocessedWebhookDto }) {
    ...
    const handleReplay = useCallback(async () => {
        await replayWebhook.mutateAsync({ provider: webhook.provider, providerEventId: webhook.providerEventId });
        setConfirmReplayOpen(false);
    }, [replayWebhook, webhook.provider, webhook.providerEventId]);
    ...
    return (
        <article className="border-b border-border py-4">
            ...
            <div className="mt-3 max-w-sm">
                <AdminIdentifier label={t('identifiers.providerEventId')} value={webhook.providerEventId} />
            </div>
