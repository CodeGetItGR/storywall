...
export function PageErrorState({ title, description, actionHref, actionLabel, onRetryAction, retryLabel, icon }: PageErrorStateProps) {
    ...
    return (
        <main className="flex min-h-dvh flex-col items-center justify-center bg-background px-6 py-16 text-center">
            <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full text-white shadow-[0_18px_45px_rgba(36,31,26,0.16)] bg-gradient-brand">
                {icon ?? <AlertTriangle className="h-7 w-7" />}
            </div>
            <h1 className="mb-3 max-w-md text-2xl font-bold text-balance text-ink lg:text-3xl">{title}</h1>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
                {onRetryAction && (
                    <button
                        ...
                        className="inline-flex min-h-11 items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-opacity bg-gradient-brand hover:opacity-90"
                    >
