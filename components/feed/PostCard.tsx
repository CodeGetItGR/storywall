...
export function PostCard({ post, showCommentLink = true, isLcpCandidate = false }: PostCardProps) {
    ...
    return (
        <article className={cn('relative border-b border-border/60 bg-card', isHostPost && 'pt-3 sm:pt-0 sm:pr-3')}>
                <div className="flex items-center justify-between px-4 pt-4 pb-3">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {isHostPost && (
                            <span aria-hidden="true" className="pointer-events-none px-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-ink-muted">
                                {t('hostPost')}
                            </span>
                        )}
                        {isMyPost && (
