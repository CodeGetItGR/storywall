        ...
        <article className="relative flex items-center gap-x-4 border-t border-border py-5 first:border-t-0">
            <div className={'flex flex-col items-center gap-x-4 gap-y-3'}>
                <div className={'flex w-full justify-start gap-5'}>
                    ...
                    <p
                        className={cn(
                            'text-md w-full leading-none font-semibold tracking-[-0.03em] text-ink tabular-nums',
                ...
                {/* Session Details */}
                <div className={cn('ml-7.5 min-w-0', hasMeta ? 'self-start pt-0.5' : 'flex min-h-9 items-center self-center')}>
                    <h2 className="text-2xl leading-tight font-semibold text-ink">{session.title}</h2>
                    {session.description && <p className="mt-1 text-sm leading-snug text-ink-muted">{session.description}</p>}

                    {session.locationName && (
                        <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] font-medium tracking-[0.14em] text-ink-muted uppercase">
                            ...
                            {session.mapsUrl && (
                                <a
                                    ...
                                    className="ml-auto inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 text-[10px] leading-none text-ink-muted transition-colors hover:border-ink/30 hover:text-ink focus-visible:ring-2 focus-visible:ring-ink/25 focus-visible:outline-none"
                                >
                                    {t('story.directions')}
