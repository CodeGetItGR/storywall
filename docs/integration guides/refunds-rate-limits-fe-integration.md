...
1. **A refund flow.** A host can ask for their activation payment back; an admin decides. Approval
   sends the money back *and takes the event down to `DRAFT`*.
...
| gate | fails when |
|---|---|
| nobody but the host joined | any non-host member has ever existed on the event, *including ones since removed* |
...
proxies for *did this host get what they paid for*, and the admin's actual job is the cases the
...
`REFUND_APPROVED` is also the only notification that reports an event *losing* its live status, so
