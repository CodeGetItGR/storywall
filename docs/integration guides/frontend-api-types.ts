 ...
 * 2026-07-30. Companion reference to docs/frontend-integration-guide.md, which covers
 ...
 */
type NotificationType = 'STORAGE_LIMIT_WARNING' | 'MEMBER_LIMIT_WARNING' | 'EVENT_CAP_WARNING' | 'UPGRADE_OFFER' | 'HOST_TIP';
...
interface NotificationResponseDto {
    id: string;
    /** All three are null for account-level notifications (e.g. EVENT_CAP_WARNING). */
...
interface EventUsageResponseDto {
    eventId: string;
    /**
     * An event's tier grants storage + member quotas; a user's tier grants the active-event cap.
     * Plan codes are no longer a fixed union — admins can create new plans at runtime via the
     * plan-tier admin endpoints, so this is `string`, not `'FREE' | 'PLUS' | 'PRO'`. The wire value
     * is unchanged (existing checks like `planTier === 'FREE'` still work); only the declared type
     * widened. See billing-fe-guide.md.
     */
    planTier: string;
    storageBytes: number;
...
/** GET /api/me/usage — self-scoped. */
interface AccountUsageResponseDto {
    userId: string;
    planTier: string;
    activeEvents: number;
...
interface CoHostInviteRequestDto {
    userId: string;
} // required
...
interface EventHostRequestDto {
    eventId: string;
    memberId: string;
    displayOrder: number;
} // all required
interface EventHostResponseDto {
    id: string;
    eventId: string;
    memberId: string;
    displayOrder: number;
...
interface EventInvitationPatchDto {
...
interface EventMemberPatchDto {
    // every field optional — isFeatured is HOST-only even on your own membership
    displayName?: string;
    nickname?: string;
...
interface EventModuleRequestDto {
    eventId: string;
    moduleKey: ModuleKey; // required — server now rejects any value outside the 5 canonical keys (400, INVALID_MODULE_KEY/3006); `ModuleKey` defined below under "App config"
    isEnabled: boolean;
    configuration: Record<string, unknown>; // both required
}
interface EventModuleResponseDto {
    ...
    createdAt: string;
    /**
     * Whether this module is actually usable: the AND of the platform kill switch, the event
     * plan's module list, and `isEnabled` above. Gate UI on this, not on `isEnabled` — a module
     * can be enabled for the event and still unavailable because the plan excludes it.
     */
    isAvailable: boolean;
}
interface EventModulePatchDto {
    isEnabled?: boolean;
    configuration?: Record<string, unknown>;
}
...
interface RsvpRequestDto {
    ...
    phone?: string; // max 50
    adultCount: number;
    childCount: number; // both required, server-validated: adults 1-5, children 0-4 (400 on violation) — see §"App config"
    notes?: string;
...
interface RsvpPatchDto {
    // every field optional
    attendanceStatus?: AttendanceStatus;
    phone?: string;
    adultCount?: number;
    childCount?: number;
    notes?: string; // same 1-5 / 0-4 bounds as RsvpRequestDto when present, null/omitted is still fine
} // no eventMemberId/submittedAt — immutable/system-managed.

// --- RSVP Session Responses (per-session attendance) ---

interface RsvpSessionResponsRequestDto {
    rsvpId: string;
...
interface MediaResponseDto {
    ...
    mimeType: string;
    mediaType: string; // mediaType free text: IMAGE | VIDEO | AUDIO | DOCUMENT by convention
    fileSize: number;
    width: number | null;
    height: number | null;
...
interface ReactionRequestDto {
    postId: string;
    memberId: string;
    reactionType: string;
} // all required, reactionType max 20
...
interface StoryRequestDto {
    eventId: string;
    authorMemberId?: string;
    mediaId: string; // mediaId required, must already exist
    caption?: string;
    songUrl?: string;
...
interface PlaylistSuggestionRequestDto {
    eventId: string;
    title: string; // title required, max 255
...
interface PlaylistSuggestionResponseDto {
    ...
    deletedAt: string | null;
    upvoteCount: number;
    downvoteCount: number; // always present; 0 for an unvoted song, not absent
...
interface PlaylistVoteRequestDto {
...
interface PlaylistSuggestionLeaderboardDto {
    ...
    createdAt: string;
    upvoteCount: number;
    downvoteCount: number; // downvoteCount breaks ties, never subtracts from rank
}

interface PostMediaRequestDto {
    postId: string;
    mediaId: string;
    displayOrder: number;
} // all required
interface PostMediaResponseDto {
    id: string;
    postId: string;
    mediaId: string;
    displayOrder: number;
...
/** Canonical event module keys — see EventModuleRequestDto.moduleKey, now server-validated against this set. */
type ModuleKey = 'posts' | 'rsvp' | 'playlist' | 'stories' | 'gallery';
...
interface AppPaginationConfigDto {
    defaultPageSize: number;
    maxPageSize: number;
}
interface AppRsvpConfigDto {
    minAdults: number;
    maxAdults: number;
    minChildren: number;
    maxChildren: number;
...
export interface PlanTierResponseDto {
    ...
    maxActiveEvents: number | null; // null = no limit enforced
...
/**
 * The `details` object on a 409 quota rejection (errorCode 5008/5009/5010). Carries the numbers
