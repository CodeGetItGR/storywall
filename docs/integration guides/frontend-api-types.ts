...
export interface PlanTierResponseDto {
  ...
  /** Module keys not in `moduleKeys` but purchasable as a `MODULE_UNLOCK` paid service on this
   *  plan. Always empty for ACCOUNT-scope plans. Only populated on `GET /api/config`. */
  paidModuleKeys: string[];
}
