...
Shipped 2026-08-26; extended 2026-09-13 to cover location. Fixes a design gap: `Event.startAt`/
`endAt` and an `EventSession`'s own `startAt`/`endAt` were two independently-writable schedules
with nothing keeping them aligned. This adds `isMain` to `EventSessionResponseDto`, makes the main
session's schedule a read-only mirror of the event's, and changes what
`PATCH /api/event-sessions/{id}` accepts for that one session. The 2026-09-13 update applies the
exact same treatment to `locationName`/`mapsUrl`, which had the identical gap.
Read this alongside
...
the main session — `title`, `description`, `displayOrder` — is unaffected and stays freely
editable, same as any other session.

**`PATCH /api/event-sessions/{id}` now also rejects `locationName`/`mapsUrl` on the main session**
(added 2026-09-13, same mechanism as the dates lock above):
...
→ 409
{
  "status": 409,
  "errorCode": 5065,
  "errorKey": "EVENT_SESSION_MAIN_LOCATION_READ_ONLY",
  "detail": "Session ... is the event's main session; edit the event's locationName/mapsUrl instead — this session's location always mirrors it."
}
```

**`PATCH /api/events/{id}` now cascades `startAt`/`endAt`/`locationName`/`mapsUrl` onto the main
...
session's `startAt`/`endAt` (and, since 2026-09-13, `locationName`/`mapsUrl`) in `sessions[]`
already updated to match. The existing event-level schedule lock (`EVENT_SCHEDULE_LOCKED`, 5048) is
unaffected and unchanged — it still gates whether the event's own `startAt`/`endAt` can move at
all; the cascade only runs after that check passes. There is no equivalent lock on the event's own
location — a host can always edit `Event.locationName`/`mapsUrl`, at any status, and the main
session follows.

Non-main sessions are never touched by an event-level patch — a satellite session (rehearsal
dinner, afterparty) keeps whatever time and location a host gave it regardless of what happens to
the event's own dates or location.

**Creation also seeds the main session's location from the event, not just its dates:** if
`initialSessionTitle` is sent at `POST /api/events`, the seeded `EventSession`'s `locationName`/
`mapsUrl` start out equal to `EventRequestDto.locationName`/`mapsUrl` (previously they started out
`null` regardless of what the event's own location was set to).

## What did not change

- `POST /api/event-sessions` — creating additional sessions is unaffected. A newly created session
  is never `isMain` (there's no field to request it), so its location and schedule are both freely
  editable like today.
- The event-level schedule lock (`EVENT_SCHEDULE_LOCKED`, 5048) and the non-main session lock
  ...
  Neither ever gated location, so there's no equivalent lock to compare the new location mirror
  against.
...
- [ ] In the session-editing UI, disable (don't just soft-validate) the start/end time inputs
      **and** the `locationName`/`mapsUrl` inputs when `session.isMain === true` — point the host
      at the event's own date/location fields instead of letting them hit a 409. Every other field
      on that form (`title`, `description`, `displayOrder`) stays enabled.
- [ ] Handle `errorCode: 5055` / `EVENT_SESSION_MAIN_DATES_READ_ONLY` and `errorCode: 5065` /
      `EVENT_SESSION_MAIN_LOCATION_READ_ONLY` defensively wherever a session PATCH is fired from a
      generic form, in case the disabled-input guard above is bypassed (bulk edit, stale cached
      `isMain`, etc.) — surface the `detail` message rather than a generic save-failed toast.
- [ ] After a successful `PATCH /api/events/{id}` that changed `startAt`/`endAt`/`locationName`/
      `mapsUrl`, invalidate/refetch the event's `sessions[]` too if you have them cached — the main
      session's dates and/or location changed server-side even though the event PATCH response
      itself doesn't show it.
